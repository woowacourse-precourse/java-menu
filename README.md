...
메뉴 추천 과정
  ...
  3. 화,수,목,금 에 대해 월요일과 같이 반복
코치 이름 - 최소 2글자,최대 4글자
코치 최소 2명,최대 5명 식사 함께
코치 각 못먹는 메뉴 최소0개 최대 2개(,)로 구분해서 입력
  먹지 못하는 메뉴가 없으면 빈 값
  추천을 못하는 경우는 없음
카테고리 - 한 주에 최대 2개 같은것 가능
          일식,한식,중식,아시안,양식 총 5개
          일식 - 9개
          한식 - 9개
          중식 - 9개
          아시안 - 9개
...
- [ ] 코치 이름 입력 받음(,) 로 구분
  - [ ] 입력 받고, 코치 객체 생성
  - [ ] 코치 별 못먹는 메뉴 입력(최소 0개 최대 2개) - , 로 구분 입력
- [ ] 추천 시작
- [ ] 월,화,수,목,금
