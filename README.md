...
- 메인 로직
  ...
  - 코치 등록
    - 코치의 이름을 입력 받는다.
      - 코치는 최소 2명 - 최대 5명까지 입력받는다
      - 각 코치의 이름은 최소 2자에서 최대 4자이다.
      - 코치의 이름은 공백없이 콤마로 구분한다.
    - 코치가 못먹는 메뉴를 입력받는다.
      - 메뉴는 최소 0개에서 최대 2명이다
      - 메뉴는 공백없이 콤마로 구분한다
  - 메뉴 추천
    - (요일별) 카테고리 선정
      - 카테고리 선정을 위한 무작위 수를 선정한다.
        - 카테고리가 3회 이상 나올 경우 예외처리 한다.
    - (코치별) 메뉴 선정
      - 해당 카테고리를 무작위로 섞는다
      - 맨 앞의 메뉴를 추천 메뉴로 선정한다.
        - 해당 메뉴가 못먹는 메뉴일 경우 예외처리
        - 해당 메뉴가 이전에 먹은 메뉴일 경우 예외처리
  - 메뉴 출력
    - 서비스 시작 문구를 출력한다
    - 서비스 추천 결과를 출력한다.
...
- 예외처리
  - 코치의 이름 입력
    - 코치 이름이 양식(콤마 구분)에 맞지 않을 경우 예외처리
    - 코치가 2명 미만, 5명 초과 시 예외처리
    - 코치 이름이 2자 미만 4자 초과시 예외처리
  - 못먹는 메뉴 입력
    - 메뉴 입력이 양식(콤마 구분)에 맞지 않을 경우 예외처리
    - 못먹는 메뉴가 2개를 초과할 경우 예외처리
  - 카테고리 선정
    - 기존 2번 등장한 카테고리가 다시 선택될 경우 예외처리
  - 메뉴 선정
    - 선정 메뉴가 못먹는 메뉴에 해당할 경우 예외처리
    - 선정 메뉴가 이미 먹은 메뉴에 해당하면 예외처리
