...
- [ ] 각 코치들이 못먹는 메뉴들을 입력받는다.
  - [ ] [예외처리] ,로 구분하는 패턴으로 입력하지 않은 경우 예외처리
  - [ ] [예외처리] 입력받은 메뉴가 존재하지 않을 경우 예외처리
  - [ ] [예외처리] 3개 이상인 경우 예외처리

- [ ] 월, 화, 수, 목, 금에 추천할 메뉴의 카테고리를 랜덤으로 생성한다.
  - [ ] 한 카테고리가 3번 중복되면 다시 랜덤으로 생성한다.

- [ ] 생성한 카테고리에 따라 각 코치들에게 요일별로 메뉴를 랜덤으로 추천하고 저장한다.
  - [ ] 코치가 먹지 못하는 메뉴는 제외시킨다.
  - [ ] 이전에 추천한 메뉴인지 중복 검사를 한다.
