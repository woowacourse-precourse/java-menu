...
1. 입출력 요구 사항
  - 입력 
    - 코치 이름 입력
      - , 로 구분해서 입력한다 
      - 각 코치의 이름은, 2글자에서 4글자 사이이다
      - 최소 2명에서, 최대 5명까지 입력해야 하고, 아니라면 [ERROR] 입력 후 다시 뽑는다
    - 각 코치들의 못먹는거 입력
      - 처음에 코치 이름을 입력받은 대로, 물어 보고 받는다.
      - 이때, 최소 0개에서 최대 2개까지 못먹는 메뉴를 받는다. 
      - 없다면, 없는대로 공백으로 둘 것. 2개가 넘는다면 [ERROR] 입력 후 다시 받는다
  ...
  - 출력
    ...
    - 코치이름 입력 (, 로 구분)
      - "코치의 이름을 입력해 주세요."
    - OO(이)가 못 먹는 메뉴를 입력해 주세요 -> 받아와서 처리하기
    - 메뉴 추천 결과입니다.
      - [ 구분 | 월요일 | 화요일 | 수요일 | 목요일 | 금요일 ] -> 얘는 그대로 출력하기
      - [ 카테고리 | 한식 | 한식 | 일식 | 중식 | 아시안 ] -> 뽑히는 대로 할 것.
      - [ 이름 | ~~~ | ~~~ | ~~~ | ~~~ | ~~~ ] -> 받은 순서대로 내기.
...
2. 정보 초기화
   - Java Enum 을 활용한다
     - 일식, 한식, 중식, 아시안, 양식 순으로 미리 Enum 에 기재한다.
...
4. 메뉴 뽑는 기능
   - 맨 처음 입력받았던 순서대로 코치들의 메뉴를 뽑아야 한다. 
   - 추천할 메뉴는, 정해진 카테고리에 있는 메뉴를 camp.nextStep.edu.missionUtils 의 Random.shuffle() 해서 뽑아 첫번째 애로 뽑는다
     - 이때, 한 코치에게 한 주에 중복되지 않는 메뉴를 추천해야 한다.
     - 이때, 그 코치가 먹지 못하는 메뉴를 줘서는 안된다.
   
   - 추천할 수 없는 메뉴를 골랐을 경우, 다시 섞은 후 첫 번째 값을 사용해야 한다
