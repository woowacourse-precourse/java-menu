...
public class FoodInfo {
    private final List<String> japanese;//1
    private final List<String> korean;//2
    private final List<String> chinese;//3
    private final List<String> asian;//4
    private final List<String> western;//5
    ...
    public boolean isExist(String foodName){
        return japanese.contains(foodName) || korean.contains(foodName) || chinese.contains(foodName)
    ...
    public String getRandomFood(int category){
        if(category == 1)
            return getRandomJapanese();
        if(category == 2)
            return getRandomKorean();
        if(category == 3)
            return getRandomChinese();
        if(category == 4)
            return getRandomAsian();
        if(category == 5)
            return getRandomWestern();
        return null;
    }
