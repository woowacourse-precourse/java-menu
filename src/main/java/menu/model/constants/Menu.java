...
public enum Menu {
    ...
    public static String getRandomMenu(String category, List<String> hateMenus) {
        List<String> recommendedMenu = new ArrayList<>(List.of("reset"));
        for (Menu menu : Menu.values()) {
            if (menu.getMenuCategory().equals(category)) {
                String randomMenu = getMenu(menu, hateMenus, recommendedMenu);
                recommendedMenu.add(randomMenu);
                return randomMenu;
            }
        }
        return null;
    ...
    private static String getMenu(Menu menu, List<String> hateMenus, List<String> recommendedMenu) {
        while (true) {
            List<String> menus = Arrays.asList(menu.getMenu().split(", "));
            String randomMenu = Randoms.shuffle(menus).get(0);
            if (getCoachFavoriteFood(randomMenu, hateMenus, recommendedMenu) != null) {
                return randomMenu;
            }
        }
    }

    private static String getCoachFavoriteFood(String randomMenu, List<String> hateMenus, List<String> recommendedMenu) {
