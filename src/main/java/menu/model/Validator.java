...
import static menu.view.ViewConstants.ERROR_MESSAGE;
...
public class Validator {
    ...
    private static void isValidCoachesSize(List<String> coachesName) {
        ...
        if (numberOfCoach < MIN_COACH_NUMBER) {
            throw new IllegalArgumentException(ERROR_MESSAGE + "코치는 최소 2명 이상 입력해야 합니다.");
        }
        if (numberOfCoach > MAX_COACH_NUMBER) {
            throw new IllegalArgumentException(ERROR_MESSAGE + "코치는 최대 5명까지 입력할 수 있습니다.");
        }
    }
    ...
    private static void isValidCoachName(List<String> coachesName) {
        for (String name : coachesName) {
            if (name.length() < MIN_NAME_LENGTH) {
                throw new IllegalArgumentException(ERROR_MESSAGE + "코치의 이름은 최소 2글자 이상 입력야 합니다.");
            }
            if (name.length() > MAX_NAME_LENGTH) {
                throw new IllegalArgumentException(ERROR_MESSAGE + "코치의 이름은 최대 4글자까지 입력할 수 있습니다.");
            }
        }
    }
    ...
    private static void isValidMenuSize(List<String> menus) {
        if (menus.size() > MAX_DISLIKE_MENU) {
            throw new IllegalArgumentException(ERROR_MESSAGE + "못 먹는 메뉴는 최대 2개까지만 입력해주세요.");
        }
    }
