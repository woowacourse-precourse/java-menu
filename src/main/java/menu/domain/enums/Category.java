...
public enum Category {
    ...
    Category(int number, String category, List<String> foods) {
        this.number = number;
    ...
    public Food mapFoodByName(String foodName) {
        return this.foods.stream()
                .filter(it -> it.name().equals(foodName))
                .findAny()
                .orElseThrow(() -> new IllegalArgumentException("해당하는 음식이 존재하지 않습니다"));
    }
}
