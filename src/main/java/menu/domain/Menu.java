...
public enum Menu {
    ...
    WE_1(WESTERN, "라자냐"), WE_2(WESTERN, "그라탕"), WE_3(WESTERN, "뇨끼"), WE_4(WESTERN, "끼슈"), WE_5(WESTERN, "프렌치 토스트"), WE_6(WESTERN, "바게트"), WE_7(WESTERN, "스파게티"), WE_8(WESTERN, "피자"), WE_9(WESTERN, "파니니");
    ...
    public static List<String> getAllMenuNamesOfCategory(Category category) {
        List<String> recommendedMenuNames = new ArrayList<>();
        Arrays.stream(values())
                .filter(menu -> menu.category == category)
                .collect(Collectors.toList())
                .forEach(menu -> recommendedMenuNames.add(menu.getName()));
