...
public enum Category {
    ...
    JAPANESE("일식", 1, new FoodRepository(getFoods(
            List.of("규동", "우동", "미소시루", "스시", "가츠동",
                    "오니기리", "하이라이스", "라멘", "오코노미야끼")
    )), Controller -> {}),
    KOREAN("한식", 2, new FoodRepository(getFoods(List.of(
            "김밥", "김치찌개", "쌈밥", "된장찌개", "비빔밥", "칼국수", "불고기",
            "떡볶이", "제육볶음"
    ))), Controller -> {}),
    CHINESE("중식", 3, new FoodRepository(getFoods(List.of(
            "깐풍기", "볶음면", "동파육", "짜장면", "짬뽕", "마파두부", "탕수육",
            "토마토 달걀볶음", "고추잡채"
    ))), Controller -> {}),
    ASIAN("아시안", 4, new FoodRepository(getFoods(List.of(
                    "팟타이", "카오 팟", "나시고렝", "파인애플 볶음밥", "쌀국수",
                    "똠얌꿍", "반미", "월남쌈", "분짜"
    ))), Controller -> {}),
    EUROPEAN("양식", 5, new FoodRepository(getFoods(List.of(
    ...
    Category(String name, int number, FoodRepository foodRepository, Consumer<Controller> consumer) {
        this.name = name;
        this.number = number;
    ...
    public static Category from(final int number) {
        return Arrays.stream(Category.values())
                ...
                .findAny()
                .orElseThrow(() -> new IllegalStateException("[ERROR] 숫자에 해당하는 음식 카테고리가 없습니다."));
    }
