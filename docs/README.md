...
- 요구사항을 모두 만족한 후 순차적으로 코드를 분리하고 테스트 코드를 작성한다
- 주어진 시간 내에 요구사항을 모두 분석하고, 만족시키는 연습을 한다
...
3. 한 요일의 추천 음식 카테고리를 선정한다.
4. 해당 요일의 카테고리의 음식들 중, 각 코치를 위한 음식을 추천한다.
5. 3 ~ 4의 과정을 매 요일에 대해서 반복한다.
6. 추천 결과를 출력한다
7. 프로그램을 종료한다

모든 메뉴를 추천하면 프로그램을 종료한다.
...
* **유의 사항**
    ...
    * 메뉴 추천
        ...
        * **반드시 개별 요일 단위로 작업을 진행해야 한다.** (ex. 월요일의 카테고리 선택 -> 메뉴 추천 -> 화요일의 카테고리 선택 -> 메뉴 추천)
          <br/>
    * 예외 처리
